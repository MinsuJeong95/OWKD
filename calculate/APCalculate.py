...
def apCalculate(args, Fold):
    path = './' + args.inputDB + '\\' + 'testResult' + '\\' + 'gceNet' + '\\' + Fold
    ...
    for pickleFilePath in pickleFilePaths:
        ...
        for i, (key, value) in enumerate(ReIDdict.items()):
            ...
            plt.title(
                'PRCurve_' + Fold + '_' + 'gceNet' + '_' + str(key) + ' AP : ' + str(AP))
            plt.fill_between(apRecall[0:len(apRecall)], apPrecision[0:len(apPrecision)], alpha=0.2)

            plt.savefig(
                graphPath + '/PRCurve_' + Fold + '_' + 'gceNet' + '_' + str(key) + '_' + str(
                    AP) + '.png')
            np.save(numpyPath + '/apRecall_' + Fold + '_' + 'gceNet' + '_' + str(key),
                    np.array(apRecall))
            np.save(numpyPath + '/apPrecision_' + Fold + '_' + 'gceNet' + '_' + str(key),
